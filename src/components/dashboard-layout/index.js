...
const NavBar = ({
  ...
  return (
    <div
      className={`flex flex-col items-center justify-center bg-background z-20 w-full fixed bottom-0 m-0 touch-none ${isStandalone ? 'h-[8%] pb-6' : 'h-[6%]'}`}
    >
    ...
    <StartDateContext.Provider value={{ startDateState, setStartDateState }}>
      <EndDateContext.Provider value={{ endDateState, setEndDateState }}>
        <PageContext.Provider value={{ page, setPage }}>
          ...
          >
            <ScrollAreaWithPulldown
              className={`${isStandalone ? 'h-[92%]' : 'h-[94%]'} w-screen fixed top-0`}
              ref={scrollAreaRef}
              onTouchStart={handleScrollAreaFocus}
              triggerRefresh={() => {
