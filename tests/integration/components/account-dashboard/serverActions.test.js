...
describe('Account Dashboard Server Actions', () => {
  ...
  describe('setPrimaryEmail', () => {
    ...
    it('should fail with invalid email address', async () => {
      ...
      // Mock cookies without high-risk action token
      const validCookieResolution = {
        get: vi.fn((name) => {
          if (name === 'session-id') {
            return {
  ...
  describe('disable2FA', () => {
    it('should successfully disable 2FA when enabled', async () => {
      // ARRANGE
      const userId = uuidv4();
      const tenantId = uuidv4();
      const testEmail = `${userId}@test.com`;
      ...
      user.backupCodes = ['123456', '789012'];
      await userRepository.set({ userId, user });
      ...
      // Mock cookies with high-risk action token
      const validCookieResolution = {
        get: vi.fn((name) => {
          if (name === 'session-id') {
            return {
    ...
    it('should fail with no high-risk action token', async () => {
      ...
      expect(result.error).toBe('Unauthenticated');
      expect(console.log).toHaveBeenCalledWith(
        'Invalid auth token for 2FA disable'
      );
    });

    it('should fail when 2FA is already disabled', async () => {
      // ARRANGE
      const userId = uuidv4();
      const tenantId = uuidv4();
      const testEmail = `${userId}@test.com`;
      ...
      // User has 2FA disabled (default state)
      const userRepository = new UserRepository({ redisAdapter });
      const user = await userRepository.get({ userId });
      ...
      // Mock cookies with high-risk action token (can be obtained directly when 2FA disabled)
      const validCookieResolution = {
        get: vi.fn((name) => {
          if (name === 'session-id') {
            return {
      ...
      // ASSERT
      expect(result.success).toBe(false);
      expect(result.error).toBe('2FA is already disabled');
      expect(console.log).toHaveBeenCalledWith('2FA already disabled for user');
    });

    it('should succeed when 2FA is disabled and user authenticates with password only', async () => {
      // ARRANGE
      const userId = uuidv4();
      const tenantId = uuidv4();
      const testEmail = `${userId}@test.com`;
      ...
      // Enable 2FA first, then disable it to test the functionality
      const userRepository = new UserRepository({ redisAdapter });
      const user = await userRepository.get({ userId });
      user.totpEnabled = true;
      user.totpSecret = 'test-secret';
      ...
      // Disable 2FA temporarily to get the token with just password
      user.totpEnabled = false;
      await userRepository.set({ userId, user });

      // Mock cookies - high-risk action token can be obtained directly when 2FA disabled
      const validCookieResolution = {
        get: vi.fn((name) => {
          if (name === 'session-id') {
            return {
      ...
      // Re-enable 2FA to have something to disable
      user.totpEnabled = true;
      await userRepository.set({ userId, user });
    ...
    it('should fail when user is a Google user', async () => {
      // ARRANGE
      const userId = uuidv4();
      const tenantId = uuidv4();
      const testEmail = `${userId}@test.com`;
      ...
      // Set user as Google user with 2FA enabled
      const userRepository = new UserRepository({ redisAdapter });
      const user = await userRepository.get({ userId });
      ...
      expect(console.log).toHaveBeenCalledWith(
        'Google user attempted to disable 2FA'
      );
    });
  });
  ...
  describe('enable2FA', () => {
    it('should fail when user is a Google user', async () => {
      ...
      // Set user as Google user with 2FA disabled
      const userRepository = new UserRepository({ redisAdapter });
      const user = await userRepository.get({ userId });
      user.authProvider = 'google';
      user.totpEnabled = false;
      ...
      expect(console.log).toHaveBeenCalledWith(
        'Google user attempted to enable 2FA'
      );
    });
    ...
    it('should fail with no high-risk action token', async () => {
      ...
      // User has 2FA disabled (default state) and is not a Google user
      const userRepository = new UserRepository({ redisAdapter });
      const user = await userRepository.get({ userId });
      expect(user.totpEnabled).toBe(false);
      ...
      expect(result.error).toBe('Unauthenticated');
      expect(console.log).toHaveBeenCalledWith(
        'Invalid auth token for 2FA enable'
      );
    });

    it('should fail when 2FA is already enabled', async () => {
      // ARRANGE
      const userId = uuidv4();
      const tenantId = uuidv4();
      const testEmail = `${userId}@test.com`;
      ...
      // ASSERT
      expect(result.success).toBe(false);
      expect(result.error).toBe('2FA is already enabled');
      expect(console.log).toHaveBeenCalledWith('2FA already enabled for user');
    });

    it('should succeed for non-Google users', async () => {
      // ARRANGE
      const userId = uuidv4();
      const tenantId = uuidv4();
      const testEmail = `${userId}@test.com`;
      ...
      const user = await userRepository.get({ userId });
      expect(user.totpEnabled).toBe(false);
      expect(user.authProvider).toBe('email'); // Email/password user (not Google)
      ...
      expect(result.success).toBe(true);
      expect(console.log).toHaveBeenCalledWith(
        '2FA enable authenticated successfully for user:',
        userId
      );
    });
  });
